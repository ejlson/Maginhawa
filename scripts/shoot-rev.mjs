...
await p.screenshot({path:`${OUT}/rev_default.png`});
const a=await p.$('a[href="/restaurants"]'); const box=await a.boundingBox();
await p.mouse.move(box.x+box.width*0.3, box.y+box.height/2);await s(600);
await p.screenshot({path:`${OUT}/rev_hover.png`});
// restaurants page (should be plain, no intro)
const p2=await b.newPage();await p2.setViewport({width:1440,height:900});
await p2.goto("http://localhost:3000/restaurants",{waitUntil:"networkidle0"});await s(700);
await p2.screenshot({path:`${OUT}/rev_page.png`});
